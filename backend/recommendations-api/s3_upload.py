"""Загрузка фотографий рекомендаций на S3 с проверкой размера и типа."""
import os
import uuid
import base64
import boto3
...
def upload_photos_to_s3(photos_list):
    """Загружает base64-фото на S3, возвращает CDN-ссылки. URL пропускает как есть."""
    if not photos_list:
        return []
    if not isinstance(photos_list, list):
        raise ValueError("Поле photos должно быть массивом")
    if len(photos_list) > MAX_PHOTOS_COUNT:
        raise ValueError(f"Слишком много фото ({len(photos_list)}). Максимум {MAX_PHOTOS_COUNT}")
    ...
    for item in photos_list:
        ...
        file_key = f"recommendation-photos/{uuid.uuid4().hex}.{ext}"
        s3.put_object(
            Bucket='files',
            Key=file_key,
